...
LOCATION="Cluj-Napoca, RO (47°N, mediocre weather, excellent food)"
COMPANY="Nexttech"
CURRENT_GIG="Fintech"
YEARS_SHIPPED=4
...
I write Spring Boot services that handle real money for a real European bank, and I do most of it with Claude Code in one window and an OpenShift terminal in another. I treat AI as a collaborator, not a magic 8-ball — it ships code faster, it doesn't ship code blindly.
...
- **Claude Code** is my main interface. Implementation, debugging, refactoring, docs — it's faster *and* I review every line before commit.
...
- **Plugins, skills, MemPalace** — I extend Claude rather than fight it.
- **Ecosystem scouting** is a hobby and a habit. New model drops, new community MCP, new agentic workflow — I'll have read the changelog, tried the repo, and DM'd the team about it before lunch.
...
- 🤖 **Anthropic Messages API** from Spring Boot — streaming, tool use, prompt engineering as actual engineering
- 🔌 **Custom MCP servers** for dev workflows that don't exist yet
- 🏛️ **Hexagonal architecture** in production — opinions: strong, justified
- 🦀 *occasionally Rust, when nobody's looking*
...
- Eating my way through Sicily one *arancino* at a time 🍝
...
| transaction traceId startswith="Received REST request" endswith="Response for POST" maxspan=60s
| eval latency_ms=round(duration*1000,2)
| stats count avg(latency_ms) as avg_latency_ms min(latency_ms) as min_ms max(latency_ms) as max_ms by status
